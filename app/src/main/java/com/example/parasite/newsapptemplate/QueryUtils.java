...
class QueryUtils {
    ...
    public static List<ArticleItem> fetchArticleData(String requestUrl) {
        Log.i(LOG_TAG, " fetchArticleData , called.");
    ...
    private static List<ArticleItem> extractFeatureFromJson(String articleJSON) {
        ...
        try {
            ...
            for (int i = 0; i < resultsArray.length(); i++) {
                ...
                if (currentArticle.has("fields")) {
                    JSONObject object = currentArticle.getJSONObject("fields");
                    articleThumbnail = object.getString("thumbnail");
                    imageThumb = drawableFromUrl(articleThumbnail);
                } else {
                    imageThumb = null;
                }


                ArticleItem article = new ArticleItem(articleTitle, articleSection, imageThumb, url);
                articles.add(article);
            }
        } catch (JSONException e) {
            Log.e("Queryutils", "Error parsing JSON responseeee", e);
        } catch (IOException e) {
            e.printStackTrace();
        }
